...
var fullslider_params = fs.open('public/tmp_files/params', 'r');
fullslider_params = fullslider_params.read();
//options.url=options.url+"?"+fullslider_params;
...
page.open(options.url, function(status) {
    ...
    page.evaluate(function(fullslider_params) {
        var slides = decodeURIComponent(escape(atob(cleanBase64Params(fullslider_params)))).replace(/transform:/g, "-webkit-transform:");
        slides = slides.replace(/transform-origin/g, "-webkit-transform-origin");
//        slides = slides.replace(/width: auto/g, "width: 100%");
        document.getElementById("slides").innerHTML = document.getElementById("slides").innerHTML + slides;
        initializeReveal();
        $("#slides").css("margin-left", "-4.84vw");
        $("#slides").css("margin-top", "-0.98vw");
    }, fullslider_params);
...
function configure(plugin) {
    if (!options.size)
        if (typeof plugin.size === 'function')
            ...
            options.size = {width: 1805, height: 1160};
    page.viewportSize = {width: 2000, height: 1200};
    ...
    plugin.currentSlide = 1 - 1; //tricky
    plugin.totalSlides = plugin.slideCount() - 1; //tricky
    return plugin;
}
...
function previousSlide(plugin) {
    plugin.currentSlide--;
    return plugin.previousSlide();
}
...
function exportSlide(plugin) {
    ...
    var decktape = Promise.resolve()
            ...
            .then(function() {
                if (plugin.currentSlideIndex() != "/null_slide") { //tricky
